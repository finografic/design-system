...
- **`variant.palette` + `colorPalette` (multi-slot):** For **Switch**, **Checkbox**, and similar, each palette key sets **`colorPalette`** on **every slot** that needs tokens (**`root`**, **`control`**, **`thumb`**, …), not only root; state styles use **`colorPalette.*`** so **`palette="info"`** (etc.) drives thumb/control. **`palette="default"`** maps to neutral/grey — use **`primary`**, **`success`**, … for saturated track colors. **Touch:** on **`sm` / `md`**, use **`@media (pointer: coarse)`** to scale control/thumb — see **`switch.recipe.ts`** and **Checkbox**.
