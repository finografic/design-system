...
- **`variant.palette` + `colorPalette` (multi-slot):** For components like **Switch**, each palette key sets **`colorPalette`** on **each slot that needs it** (`root`, `control`, `thumb`, …), not only on root. Base and state keys (`_checked`, etc.) then use **`colorPalette.*`** (e.g. `light` / `base` / `dark`) so the **same** implementation picks **semantic colors for the chosen palette** (e.g. `palette="info"` → info tokens on thumb/control). See **`switch.recipe.ts`** `variants.palette` as the working reference.
