...
- **Releases:** `scripts/release.ts` (`release:patch` / `minor` / `major`) runs **`pnpm lint:md`**, bumps versions, runs **`pnpm build:all`**, commits **`package.json` + `dist/`** in one release commit, then tags/publishes/pushes. Use **`--ignore-scripts`** on version bump and publish so `prepare` / lifecycle hooks do not rewrite the tree mid-release. Stop **`pnpm watch`** before releasing.
