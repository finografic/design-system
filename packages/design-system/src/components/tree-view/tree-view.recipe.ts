...
export const treeViewRecipe = sva({
  ...
  base: {
    ...
    branchControl: {
      ...
      'color': 'fg',
      'textAlign': 'start',
      // depth-based padding — vars set on this element, `--depth` injected by Ark
      ...
      '_selected': { bg: 'bg.muted', color: 'accent.fg' },
      '_disabled': { opacity: 0.5, filter: 'grayscale(100%)', cursor: 'not-allowed' },
    },
    ...
    branchIndentGuide: {
      ...
      'position': 'absolute',
      'zIndex': '1',
      // mirrors the horizontal offset math of branchControl
      '--tree-depth': 'calc(var(--depth) - 1)',
      '--tree-indentation-offset': 'calc(var(--tree-indentation) * var(--tree-depth))',
    ...
    item: {
      ...
      'textAlign': 'start',
      'textDecoration': 'none',
      // mirrors branchControl depth calculation
      '--tree-depth': 'calc(var(--depth) - 1)',
      '--tree-indentation-offset': 'calc(var(--tree-indentation) * var(--tree-depth))',
      '--tree-icon-offset': 'calc(var(--tree-icon-size) * var(--tree-depth) * 0.5)',
      '--tree-offset':
...
export type TreeViewRecipeProps = RecipeProps<typeof treeViewRecipe>;
