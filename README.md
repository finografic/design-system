...
| `@workspace/design-system/forms` | Form inputs (Checkbox, Switch, Select, SelectSearchable, InputField, RadioGroup, Slider) |
| `@workspace/design-system/forms/primitives` | Raw Ark re-exports — escape hatch, no DS styling |
...
| --- | --- |
| `Checkbox` / `CheckboxField` | Ark Checkbox with label + description |
| `Switch` / `SwitchField` | Ark Switch with label |
| `Select` | Ark Select — Root, Trigger, Content, Item, etc. |
| `SelectSearchable` | Searchable select with match-sorter |
| `InputField` | Composed input with label, description, error |
| `RadioGroup` | Ark RadioGroup with card-style options |
| `Slider` | Ark Slider — Root, Track, Range, Thumb, Label |
...
| `formFieldRecipe` | `formField` | root · label · helperText · errorText |
...
| `selectRecipe` | `select` | root · control · trigger · content · item · itemGroup · label · indicator |
| `switchRecipe` | `switch` | root · control · thumb · label |
...
│   └── tooltip/
│
├── forms/                   ← Pre-composed form inputs
│   ├── checkbox/
│   ├── select/
│   ├── switch/
│   └── _v1/                 ← Legacy (InputField, RadioGroup, Slider, Select)
│
